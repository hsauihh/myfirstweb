...
> 状态：**阶段 1 已实现**（`RAGdata/` 入库 + 本地 fastembed 向量化 + 余弦检索 + 注入提示词 + 独立额度），并已实现**个人知识库**（登录用户在 `/knowledge` 把博客文章选入自己的知识库、来源管理、懒同步，问答时与站内公共库合并，支持「问答/上下文」模式与「使用系统知识库」开关），见 README「知识库（RAG）」。本文保留完整方案与后续阶段。
