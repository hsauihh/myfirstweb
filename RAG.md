...
> 状态：**阶段 1 已实现**（`RAGdata/` 入库 + 本地 fastembed 向量化 + 余弦检索 + 注入提示词 + 独立额度），见 README「知识库（RAG）」。本文保留完整方案与后续阶段。
...
| 阶段 2 | 增量入库、按来源过滤、检索结果去重 |
| 阶段 3 | 重排（rerank）、混合检索（关键词 + 向量）、引用高亮 |
